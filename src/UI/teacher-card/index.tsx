...
export const TeacherCard = ({ data, studyPhase }: TeacherCardProps) => {
  return (
    <Link href={`/teachers/${data._id}`}>
      <div className='flex flex-col aspect-[4/5] w-[380px] border rounded-lg shadow-md cursor-pointer hover:bg-[#f8f8f8] hover:scale-[1.01] duration-300'>
        <div className='w-full h-[200px] relative overflow-hidden rounded-t-lg'>
          <Image width={380} height={200} alt='' src={data?.profileImage ?? ''} />
        </div>
